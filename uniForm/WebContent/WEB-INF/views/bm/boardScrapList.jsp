...
%>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				// ====== 스크랩취소 버튼 클릭 이벤트 ==============
				$("#scrapOff_btn").click(function(){
					console.log('스크랩취소 버튼 클릭! >>> 스크랩 취소 컨트롤러로 이동');
			...
			function goBoardScrapList(page){
				console.log("goBoardScrapList 함수 호출! >>> page : " + page);
				...
				.attr({
					"method":"post",
					"action":"/board/boardScrapList.uni"
				}).submit();
			} // end of goBoardList 함수
			
			function goScrapOff(bm_no,i_no){
				$.ajax({
					url:"/board/scrapOff.uni",
					type:"POST",
					data:{
						bm_no:bm_no,
						i_no:i_no
					...
					error:function(request,status,error){
						alert("에러났어요 >>> : " + request.status + " : error=" + error);
					}
				});
			}
...
%>							
	...
	<div id="box_wrap_body" class="wrap1200">
		...
		<div id="box_sub">
			...
			<div id="box_sidebar">
				...
				<nav>
					<ul>		
						...
						<li class="on"><a href="/board/boardScrapList.uni?i_no=<%=i_no %>">스크랩글</a></li>
						<li><a href="#">인기글</a></li>
					</ul>
				</nav>
				<!-- // SNB -->
			...
			<%
				List<BmBoardVO> boardScrapList = (List<BmBoardVO>)request.getAttribute("boardScrapList");	
				...
				<article id="skipnav">
					...
					<section class="title_cont">
						...
						<section class=""><!-- .box_instruc_con -->
							...
							<section>
								...
								<div class="table_wrap alignC marB20">
									...
									<form id="scrapForm" name="searchForm">
										...
										<tbody>
									<%
										if(boardScrapList!=null && boardScrapList.size()>0){
											for(int i=0;i<boardScrapList.size();i++){
												BmBoardVO bbvo = boardScrapList.get(i);
							...
							<!-- ============== 스크랩삭제 버튼 출력 ============== -->
							<div class="marB40">
								<input type="button" class="grey_btn" id="scrapOff_btn" value="스크랩삭제">
							</div> <!-- end of 스크랩삭제  -->
