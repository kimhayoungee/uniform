...
<%
		...
		if(boardDetailList!=null && boardDetailList.size()>0){
				bbvo = boardDetailList.get(0);
				bm_category = bbvo.getBm_category();
				
				System.out.println("bm_category >>> 수정폼 : " + bm_category);
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$("#update_btn").click(function(){
					...
					// 수정 컨트롤러 거친 후 상세보기로 이동
					$("#updateForm").attr({
						"method":"POST",
						"enctype":"multipart/form-data",
						"action":"/board/boardUpdate.uni"
					}).submit();
				...
				$("#boardDetail").click(function(){
					console.log('취소 버튼 클릭! >>> boardDetail로 이동');
...
%>							
	...
	<div id="box_wrap_body" class="wrap1200">
		...
		<form id="updateForm" name="updateForm">
			...
			<input type="hidden" id="old_img" name="old_img" value="<%=bbvo.getBm_image() %>">
			<input type="hidden" id="old_file" name="old_file" value="<%=bbvo.getBm_file() %>">
					...
					<tr>
				    	...
				    	<td scope="col" style="padding:1px 1px;">
				    		<div class="insert_subject">
				    			<input type="text" name="bm_subject" id="bm_subject" value="<%=bbvo.getBm_subject() %>" style="width:99%;height:100%;" placeholder="제목을 입력해주세요">
							</div>
						</td>
					</tr>
					<tr>
						<td colspan="4">
							<textarea name="bm_context" id="bm_context" style="width:100%;" rows="10"><%=bbvo.getBm_context() %></textarea>
						</td>
					</tr>
					<tr>	
						<th>이미지</th>
		...
		<fieldset>
			<div class="marB100">
				...
				<input type="button" class="grey_btn" id="boardDetail" value="취소">
			</div>
		</fieldset>	
	</form>					
<!-- =========================================== end of 각자 집어넣을 내용 ================================================ -->	
